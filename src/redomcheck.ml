...
(* Cache lattice extended with generators *)
module CacheLat(L: LATTICE) =
struct
  let name = "Cachelattice(" ^ L.name ^ ")"
  include Cache.Make(L)

  let arb_str = Arb.among [0;1;2;3;4;5;6;7](*Arb.string*)
  ...
  (*  build_smap :  (string * L.elem) list -> (L.elem IMap.t) Arb.t *)
  let build_smap = build_map IMap.empty IMap.add
  let arb_elem_le st = match st with
    | Nonbot st ->
      Arb.(lift
	     (fun st -> Nonbot st)
	     (le_gen (IMap.bindings st) Arb.(fun es -> le_entries L.arb_elem_le es >>= build_smap)))
    | Exttop st ->
      if IMap.is_empty st
      then arb_elem
      else 
	Arb.(choose
		...
		(le_gen (IMap.bindings st) Arb.(fun es -> le_entries L.arb_elem_le es >>= build_smap));
		 ...
		 (fix ~max:10 ~base:(return st)
		    (fun stgen -> arb_str >>= fun s ->
		      if IMap.mem s st
		      then lift3 IMap.add (return s) (L.arb_elem_le (IMap.find s st)) stgen
		                    (* generate entry less than present *)
		      else lift3 IMap.add (return s) L.arb_elem stgen)); ])

 let equiv_pair = Arb.(arb_elem >>= fun st -> match st with
			 | Nonbot st ->
 			   permute (IMap.bindings st) >>= fun perm_entries ->
			    (build_smap perm_entries) >>= fun perm_st ->
			     return (Nonbot st,Nonbot perm_st)
			 | Exttop st ->
 			   permute (IMap.bindings st) >>= fun perm_entries ->
...
module ParityCache = CacheLat(Parity)
module SignCache = CacheLat(Sign)
module ConstpropCache = CacheLat(Constprop)
module ModCache = CacheLat(Modulo)
module IntervalCache = CacheLat(Interval)
...
struct
  ...
  let test_l_widening =
    let widening = (L.name^".d",L.widening) in
    [testsig (module L) -!-> (module L) ---> (module L) =: widening;
     testsig (module L) -~-> (module L) ---> (module L) =: widening;
     testsig (module L) ---> (module L) -!-> (module L) =: widening;
     testsig (module L) ---> (module L) -~-> (module L) =: widening;]

  let test_l_narrowing =
    let pp = PP.pair L.to_string L.to_string in
    mk_test ~n:1000 ~pp:pp ~limit:1 ~size:(fun p -> String.length (pp p))
            ~name:("narrowing less than first argument in " ^ L.name)
      (Arb.pair L.arb_elem L.arb_elem)
      (fun (l,l') -> L.leq (L.narrowing l l') l)

  let test_l_narrowing_keepabove =
    let pp = PP.triple L.to_string L.to_string L.to_string in
    mk_test ~n:1000 ~pp:pp ~limit:1 ~size:(fun t -> String.length (pp t))
            ~name:("narrowing keeps above lower bound in " ^ L.name)
      Arb.( pair L.arb_elem L.arb_elem >>= fun (y,z) -> return (L.meet y z, y, z))
      (fun (x,y,z) -> Prop.assume (L.leq x y);
	              Prop.assume (L.leq x z);
 	              L.leq x (L.narrowing y z))
  ...
  let test_widening op_desc =
    [testsig (module RegL) -!-> (module RegL) ---> (module RegL) =: op_desc;
     testsig (module RegL) -~-> (module RegL) ---> (module RegL) =: op_desc;
     testsig (module RegL) ---> (module RegL) -!-> (module RegL) =: op_desc;
     testsig (module RegL) ---> (module RegL) -~-> (module RegL) =: op_desc;]

  let widening0_rev r r' = RegL.rev (RegL.widening0 (RegL.rev r) (RegL.rev r'))
  let widening1_rev r r' = RegL.rev (RegL.widening1 (RegL.rev r) (RegL.rev r'))
  let widening0_det r r' = RegL.determinize (RegL.widening0 r r')
  let widening1_det r r' = RegL.determinize (RegL.widening1 r r')
  ...
  let test_brzozowski_id =  (* forall r. r <= sum_{e in range(r)} concat (e, d (rep e) (r))  *)
    mk_test ~n:1000 ~pp:RegL.to_string ~limit:1 ~size:(fun r -> String.length (RegL.to_string r))
            ~name:("Brzozowski identity in " ^ L.name)
      RegL.arb_elem
      (fun r -> let range = RegL.range r in
		let sum = L.fold_partition
		  (fun acc eqcl ->
 		     let e = L.project eqcl in
		     let repr = L.repr eqcl in
		     RegL.union (RegL.concat (RegL.letter e, RegL.d repr r),acc))
		            (if RegL.nullable r then Regexpdom.Eps else Regexpdom.Empty)
		  range in
		RegL.leq r sum)
  ...
  let suite = flatten [
    ...
    test_union;
    test_inter;
    test_widening (RegL.name^".widening0", RegL.widening0);
    test_widening (RegL.name^".widening1", RegL.widening1);    (* aka widening *)
    test_widening (RegL.name^".widening0_rev", widening0_rev);
    test_widening (RegL.name^".widening1_rev", widening1_rev);
    (*test_widening (RegL.name^".widening0_det", widening0_det);*)  (* determinizing *)
    (*test_widening (RegL.name^".widening1_det", widening1_det);*)  (*  takes LONG *)
    test_rev;
    [test_rev_rev_id;
     test_brzozowski_id]; ]
end
...
struct
  ...
  let arb_elem = Arb.among [0;1;2;3;4;5;6;7] (* Arb.string *)
  let bg_repr = 0
  let succ = succ
...
module ParityCacheTests = GenericTests(ParityCache)
module SignCacheTests = GenericTests(SignCache)
module ConstpropCacheTests = GenericTests(ConstpropCache)
module ModCacheTests = GenericTests(ModCache)
module IntervalCacheTests = GenericTests(IntervalCache)

(* Operation tests *)
module IntCache = IntervalCache
let test_lookup =
  let ac_lookup = (IntCache.name^".lookup", IntCache.lookup) in
  [pw_right (module IntTag) op_monotone  (module IntCache) (module Interval) =:: ac_lookup;
   pw_right (module IntTag) op_strict    (module IntCache) (module Interval) =:: ac_lookup;
   pw_right (module IntTag) op_invariant (module IntCache) (module Interval) =:: ac_lookup;]

let test_extend = (* Current extend implementation is not strict in third argument *)
  let as_extend = (IntCache.name^".extend", IntCache.extend) in
  [pw_right (module IntTag) (pw_right (module Interval) op_monotone)  (module IntCache) (module IntCache) =:: as_extend;
   (*pw_right (module IntTag) (pw_right (module Interval) op_strict)  (module IntCache) (module IntCache) =:: as_extend;*)
   pw_right (module IntTag) (pw_right (module Interval) op_invariant) (module IntCache) (module IntCache) =:: as_extend;
   pw_left (module IntCache) (pw_left (module IntTag) op_monotone)  (module Interval) (module IntCache) =:: as_extend;
   (*pw_left (module IntCache) (pw_left (module IntTag) op_strict)    (module Interval) (module IntCache) =:: as_extend;*)
   pw_left (module IntCache) (pw_left (module IntTag) op_invariant) (module Interval) (module IntCache) =:: as_extend;]

(* Widening tests *)
let test_ic_widening =
  let ic_widening = (IntCache.name^".widening", IntCache.widening) in
  [testsig (module IntCache) -!-> (module IntCache) ---> (module IntCache) =: ic_widening;
   testsig (module IntCache) -~-> (module IntCache) ---> (module IntCache) =: ic_widening;
   testsig (module IntCache) ---> (module IntCache) -!-> (module IntCache) =: ic_widening;
   testsig (module IntCache) ---> (module IntCache) -~-> (module IntCache) =: ic_widening;]
...
let _ =
  ...
  then
	...
	ParityCacheTests.suite;
	SignCacheTests.suite;
	ConstpropCacheTests.suite;
	ModCacheTests.suite;
        IntervalCacheTests.suite;   (* infinite number of atoms *)
	(* arithmetic tests *)
